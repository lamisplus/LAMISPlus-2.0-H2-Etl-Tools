...
package lamisplus_etl.prep_patient_h2_6_0;
...
/**
 * Job: PrEP_Patient_H2 Purpose: Patient Workflow with validated FHIR R4 and JSON Fields and Codeset transfomations<br>
 * Description: Validate JSON Fields and Codeset transfomation <br>
 * @author ilozuechukwuemeka@yahoo.com
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class PrEP_Patient_H2 implements TalendJob {
...
public java.lang.String getLamish2_Password(){
	...
	private final String jobName = "PrEP_Patient_H2";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				PrEP_Patient_H2.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(PrEP_Patient_H2.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class CreatJSON_FieldStruct implements routines.system.IPersistableRow<CreatJSON_FieldStruct> {
    final static byte[] commonByteArrayLock_LAMISPLUS_ETL_PrEP_Patient_H2 = new byte[0];
    static byte[] commonByteArray_LAMISPLUS_ETL_PrEP_Patient_H2 = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_LAMISPLUS_ETL_PrEP_Patient_H2.length) {
				if(length < 1024 && commonByteArray_LAMISPLUS_ETL_PrEP_Patient_H2.length == 0) {
   					commonByteArray_LAMISPLUS_ETL_PrEP_Patient_H2 = new byte[1024];
				} else {
   					commonByteArray_LAMISPLUS_ETL_PrEP_Patient_H2 = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_LAMISPLUS_ETL_PrEP_Patient_H2, 0, length);
			strReturn = new String(commonByteArray_LAMISPLUS_ETL_PrEP_Patient_H2, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_LAMISPLUS_ETL_PrEP_Patient_H2) {
...
CreatJSON_Field_tmp.first_name = (Patients_mapping.other_names==null)||("".equals(Patients_mapping.other_names))?null:Patients_mapping.other_names.trim() ;
CreatJSON_Field_tmp.surname = (Patients_mapping.surname==null)||("".equals(Patients_mapping.surname))?null:Patients_mapping.surname.trim() ;
...
CreatJSON_Field_tmp.deceased = false;
...
System.out.println("Total filtered PrEP patients fetched from LAMIS3 DB - "+globalMap.get("tDBInput_17_NB_LINE"));
...
System.out.println("Total updated records migrated - "+globalMap.get("tDBOutput_1_NB_LINE_UPDATED"));
...
System.out.println("*************PrEP PATIENT MIGRATION REPORT END*****************");
System.out.println("**********************************************************");
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row5";
		...
		int tos_count_tAdvancedHash_row5 = 0;
			   		...
			   		// source node:tDBInput_15 - inputs:(after_tDBInput_17) outputs:(row5,row5) | target node:tAdvancedHash_row5 - inputs:(row5) outputs:()
			   		// linked node: tMap_9 - inputs:(Unique_Patients,row5,row6) outputs:(final_mapper)
 ...
 */
	...
	final String decryptedPassword_tDBInput_15 = context.LAMISPlus_Password; 
		    ...
		    String dbquery_tDBInput_15 = "SELECT CASE WHEN MAX(id) IS NULL THEN 0 ELSE MAX(id) END AS max FROM patient_person";
            ...
            row5.max = rs_tDBInput_15.getInt(1);
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row6";
		...
		int tos_count_tAdvancedHash_row6 = 0;
			   		...
			   		// source node:tDBInput_19 - inputs:(after_tDBInput_17) outputs:(row6,row6) | target node:tAdvancedHash_row6 - inputs:(row6) outputs:()
			   		// linked node: tMap_9 - inputs:(Unique_Patients,row5,row6) outputs:(final_mapper)
 ...
 */
	...
	final String decryptedPassword_tDBInput_19 = context.LAMISPlus_Password; 
		    ...
		    String dbquery_tDBInput_19 = "SELECT  uuid, id FROM patient_person";
        	...
        	row6.uuid = routines.system.JDBCUtil.getString(rs_tDBInput_19, 1, false);
		                    }
							if(colQtyInRs_tDBInput_19 < 2) {
								row6.id = 0;
							} else {
		                          
            row6.id = rs_tDBInput_19.getInt(2);
 ...
 */
	...
	final String decryptedPassword_tDBInput_11 = context.LAMIS3_Password; 
		    ...
		    String dbquery_tDBInput_11 = "SELECT p.prepscreening_id as patient_id, CONCAT(p.prepscreening_id, '-', ndrf.datim_id, '-', p.prepscreening_id) as per"
+"son_uuid,\np.gender,p.marital_status,\n    p.education,p.occupation,\n	'' as next_of_kin_relationship\nFROM prepscreenin"
+"g p\nJOIN facility ndrf ON ndrf.facility_id = p.facility_id\nJOIN\npublic.dblink\n('lamis','SELECT base_org.name, base_o"
+"rg.id, org.code\nFROM base_organisation_unit base_org\nINNER JOIN base_organisation_unit_identifier org\nON org.organisa"
+"tion_unit_id=base_org.id WHERE organisation_unit_level_id=4'\n)\nAS lamisplus(name VARCHAR, id bigint, code VARCHAR)\nON"
+" ndrf.datim_id = lamisplus.code\nORDER BY p.prepscreening_id ASC";
...
if(dbschema_tDBOutput_3 == null || dbschema_tDBOutput_3.trim().length() == 0) {
	tableName_tDBOutput_3 = ("etl_prep_patient_stage");
} else {
	tableName_tDBOutput_3 = dbschema_tDBOutput_3 + "\".\"" + ("etl_prep_patient_stage");
}
...
int count_tDBOutput_3=0;
                                ...
                                try (java.sql.ResultSet rsTable_tDBOutput_3 = dbMetaData_tDBOutput_3.getTables(null, null, null, new String[]{"TABLE"})) {
                                    ...
                                    while(rsTable_tDBOutput_3.next()) {
                                        ...
                                        if(table_tDBOutput_3.equals(("etl_prep_patient_stage"))
                                ...
                                if(!whetherExist_tDBOutput_3) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_3 = conn_tDBOutput_3.createStatement()) {
                                        stmtCreate_tDBOutput_3.execute("CREATE TABLE \"" + tableName_tDBOutput_3 + "\"(\"patient_id\" SERIAL  not null ,\"person_uuid\" VARCHAR(36)  ,\"gender\" VARCHAR(7)  ,\"marital_status\" VARCHAR(50)  ,\"education\" VARCHAR(50)  ,\"occupation\" VARCHAR(50)  ,\"next_of_kin_relationship\" VARCHAR(50)  ,primary key(\"person_uuid\"))");
                                    }
                                }
...
for(int i_tSortRow_1_SortIn = 0; i_tSortRow_1_SortIn < array_tSortRow_1_SortIn.length; i_tSortRow_1_SortIn++){
	...
	row3.patient_id = current_tSortRow_1_SortIn.patient_id;
	row3.person_uuid = current_tSortRow_1_SortIn.person_uuid;
	row3.gender = current_tSortRow_1_SortIn.gender;
	row3.marital_status = current_tSortRow_1_SortIn.marital_status;
	row3.education = current_tSortRow_1_SortIn.education;
	row3.occupation = current_tSortRow_1_SortIn.occupation;
	row3.next_of_kin_relationship = current_tSortRow_1_SortIn.next_of_kin_relationship;
...
} else {pstmtUpdate_tDBOutput_3.setString(1, row3.gender);
}

                    if(row3.marital_status == null) {
pstmtUpdate_tDBOutput_3.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(2, row3.marital_status);
}

                    if(row3.education == null) {
pstmtUpdate_tDBOutput_3.setNull(3, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(3, row3.education);
}

                    if(row3.occupation == null) {
pstmtUpdate_tDBOutput_3.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(4, row3.occupation);
}

                    if(row3.next_of_kin_relationship == null) {
pstmtUpdate_tDBOutput_3.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(5, row3.next_of_kin_relationship);
}


                    pstmtUpdate_tDBOutput_3.setInt(6 + count_tDBOutput_3, row3.patient_id);


                    if(row3.person_uuid == null) {
pstmtUpdate_tDBOutput_3.setNull(7 + count_tDBOutput_3, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_3.setString(7 + count_tDBOutput_3, row3.person_uuid);
}
            ...
            if(updateFlag_tDBOutput_3 == 0) {
                        ...
                        pstmtInsert_tDBOutput_3.setInt(1, row3.patient_id);

                        if(row3.person_uuid == null) {
pstmtInsert_tDBOutput_3.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(2, row3.person_uuid);
}

                        if(row3.gender == null) {
pstmtInsert_tDBOutput_3.setNull(3, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(3, row3.gender);
}

                        if(row3.marital_status == null) {
pstmtInsert_tDBOutput_3.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(4, row3.marital_status);
}

                        if(row3.education == null) {
pstmtInsert_tDBOutput_3.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(5, row3.education);
}

                        if(row3.occupation == null) {
pstmtInsert_tDBOutput_3.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(6, row3.occupation);
}

                        if(row3.next_of_kin_relationship == null) {
pstmtInsert_tDBOutput_3.setNull(7, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_3.setString(7, row3.next_of_kin_relationship);
}
...
System.out.println("Total PrEP patients fetched from LAMIS3 DB - "+globalMap.get("tDBInput_11_NB_LINE"));
...
public static class row4Struct implements routines.system.IPersistableRow<row4Struct> {
	...
	public void copyDataTo(row4Struct other) {
		...
		other.id = this.id;
	            other.hospital_num = this.hospital_num;
	            other.uuid = this.uuid;
	            other.surname = this.surname;
	            other.other_names = this.other_names;
	            other.gender = this.gender;
	            other.date_birth = this.date_birth;
	            other.dob_estimated = this.dob_estimated;
	            other.education = this.education;
	            other.marital_status = this.marital_status;
	            other.occupation = this.occupation;
	            other.address = this.address;
	            ...
	            other.next_of_kin_phone = this.next_of_kin_phone;
	            other.next_of_kin_relationship = this.next_of_kin_relationship;
	            other.date_registration = this.date_registration;
	            other.archived = this.archived;
...
if(dbschema_tDBOutput_4 == null || dbschema_tDBOutput_4.trim().length() == 0) {
	tableName_tDBOutput_4 = ("etl_prep_patient_stage_2");
} else {
	tableName_tDBOutput_4 = dbschema_tDBOutput_4 + "\".\"" + ("etl_prep_patient_stage_2");
}
...
int count_tDBOutput_4=0;
                                ...
                                try (java.sql.ResultSet rsTable_tDBOutput_4 = dbMetaData_tDBOutput_4.getTables(null, null, null, new String[]{"TABLE"})) {
                                    ...
                                    while(rsTable_tDBOutput_4.next()) {
                                        ...
                                        if(table_tDBOutput_4.equals(("etl_prep_patient_stage_2"))
                                ...
                                if(!whetherExist_tDBOutput_4) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_4 = conn_tDBOutput_4.createStatement()) {
                                        stmtCreate_tDBOutput_4.execute("CREATE TABLE \"" + tableName_tDBOutput_4 + "\"(\"id\" INT8  not null ,\"hospital_num\" VARCHAR(25)   not null ,\"uuid\" VARCHAR(100)   not null ,\"surname\" TEXT ,\"other_names\" TEXT ,\"gender\" VARCHAR(7)  ,\"date_birth\" DATE ,\"dob_estimated\" BOOL ,\"education\" VARCHAR(25)  ,\"marital_status\" VARCHAR(15)  ,\"occupation\" VARCHAR(25)  ,\"address\" TEXT ,\"phone\" TEXT ,\"next_of_kin\" TEXT ,\"next_of_kin_address\" TEXT ,\"next_of_kin_phone\" TEXT ,\"next_of_kin_relationship\" VARCHAR(25)  ,\"date_registration\" DATE  not null ,\"archived\" INT4  not null ,\"last_modified\" TIMESTAMP(29)  ,\"facility_id\" INT8  not null ,\"facility_name\" VARCHAR(100)   not null ,\"lga_name\" VARCHAR(45)   not null ,\"datim_id\" VARCHAR(45)   not null ,primary key(\"uuid\"))");
                                    }
                                }
	    ...
	    String insert_tDBOutput_4 = "INSERT INTO \"" + tableName_tDBOutput_4 + "\" (\"id\",\"hospital_num\",\"uuid\",\"surname\",\"other_names\",\"gender\",\"date_birth\",\"dob_estimated\",\"education\",\"marital_status\",\"occupation\",\"address\",\"phone\",\"next_of_kin\",\"next_of_kin_address\",\"next_of_kin_phone\",\"next_of_kin_relationship\",\"date_registration\",\"archived\",\"last_modified\",\"facility_id\",\"facility_name\",\"lga_name\",\"datim_id\") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	    ...
	    String update_tDBOutput_4 = "UPDATE \"" + tableName_tDBOutput_4 + "\" SET \"id\" = ?,\"hospital_num\" = ?,\"surname\" = ?,\"other_names\" = ?,\"gender\" = ?,\"date_birth\" = ?,\"dob_estimated\" = ?,\"education\" = ?,\"marital_status\" = ?,\"occupation\" = ?,\"address\" = ?,\"phone\" = ?,\"next_of_kin\" = ?,\"next_of_kin_address\" = ?,\"next_of_kin_phone\" = ?,\"next_of_kin_relationship\" = ?,\"date_registration\" = ?,\"archived\" = ?,\"last_modified\" = ?,\"facility_id\" = ?,\"facility_name\" = ?,\"lga_name\" = ?,\"datim_id\" = ? WHERE \"uuid\" = ?";
 ...
 */
	...
	final String decryptedPassword_tDBInput_13 = context.LAMIS3_Password; 
		    ...
		    String dbquery_tDBInput_13 = "SELECT DISTINCT ps.prepscreening_id as id,ps.client_unique_id AS hospital_num,  CONCAT(ps.prepscreening_id, '-', fc.dat"
+"im_id, '-', ps.prepscreening_id) as uuid,\ninitcap(replace(replace(replace(replace(replace(replace(replace(replace(repla"
+"ce(replace(replace(replace(ps.surname, '*', 'e'), '`', 'c'), '~', 'b'), '^', 'a'), '{', 'o'), '}', 'n'), '|', 'k'), '%',"
+" 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) as surname,\ninitcap(replace(replace(replace(replace(replace(replace("
+"replace(replace(replace(replace(replace(replace(ps.other_names, '*', 'e'), '`', 'c'), '~', 'b'), '^', 'a'), '{', 'o'), '"
+"}', 'n'), '|', 'k'), '%', 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) as other_names,\nps.gender, ps.date_birth,\n"
+"'0'::BOOLEAN as dob_estimated,\nps.education,ps.marital_status,ps.occupation,\ninitcap(replace(replace(replace(replace(r"
+"eplace(replace(replace(replace(replace(replace(replace(replace(ps.address, '*', 'e'), '`', 'c'), '~', 'b'), '^', 'a'), '"
+"{', 'o'), '}', 'n'), '|', 'k'), '%', 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) as address,\nreplace(replace(repl"
+"ace(replace(replace(replace(replace(replace(ps.phone, '!', '9'), '@', '8'), '#', '7'), '$', '6'), '*', '5'), '`', '3'),'"
+"~', '2'),'^', '1') as phone,\nNULL as next_of_kin,\nNULL as next_of_kin_address,\nNULL as next_of_kin_phone,\n    NULL A"
+"S next_of_kin_relationship,ps.date_visit AS date_registration,\n	'0'::INTEGER archived,\n	ps.time_stamp as last_modified"
+",\n    ps.facility_id,fc.name AS facility_name,lg.name AS lga_name, fc.datim_id AS datim_id\n  FROM public.prepscreening"
+" ps\n  INNER JOIN facility fc\n  ON ps.facility_id=fc.facility_id\n  INNER JOIN lga lg\n  ON fc.lga_id=lg.lga_id\n  ORDE"
+"R BY ps.prepscreening_id ASC";
            ...
            row4.id = rs_tDBInput_13.getLong(1);
            if(rs_tDBInput_13.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
							if(colQtyInRs_tDBInput_13 < 2) {
								row4.hospital_num = null;
							} else {
	                         		
        	row4.hospital_num = routines.system.JDBCUtil.getString(rs_tDBInput_13, 2, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 3) {
								row4.uuid = null;
							} else {
	                         		
        	row4.uuid = routines.system.JDBCUtil.getString(rs_tDBInput_13, 3, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 4) {
								row4.surname = null;
							} else {
	                         		
        	row4.surname = routines.system.JDBCUtil.getString(rs_tDBInput_13, 4, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 5) {
								row4.other_names = null;
							} else {
	                         		
        	row4.other_names = routines.system.JDBCUtil.getString(rs_tDBInput_13, 5, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 6) {
								row4.gender = null;
							} else {
	                         		
        	row4.gender = routines.system.JDBCUtil.getString(rs_tDBInput_13, 6, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 7) {
								row4.date_birth = null;
							} else {
										
			row4.date_birth = routines.system.JDBCUtil.getDate(rs_tDBInput_13, 7);
		                    }
							if(colQtyInRs_tDBInput_13 < 8) {
								row4.dob_estimated = null;
							} else {
	                         		
            row4.dob_estimated = rs_tDBInput_13.getBoolean(8);
            if(rs_tDBInput_13.wasNull()){
                    row4.dob_estimated = null;
            }
		                    }
        	...
        	row4.education = routines.system.JDBCUtil.getString(rs_tDBInput_13, 9, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 10) {
								row4.marital_status = null;
							} else {
	                         		
        	row4.marital_status = routines.system.JDBCUtil.getString(rs_tDBInput_13, 10, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 11) {
								row4.occupation = null;
							} else {
	                         		
        	row4.occupation = routines.system.JDBCUtil.getString(rs_tDBInput_13, 11, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 12) {
								row4.address = null;
							} else {
	                         		
        	row4.address = routines.system.JDBCUtil.getString(rs_tDBInput_13, 12, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 13) {
								row4.phone = null;
							} else {
	                         		
        	row4.phone = routines.system.JDBCUtil.getString(rs_tDBInput_13, 13, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 14) {
								row4.next_of_kin = null;
							} else {
	                         		
        	row4.next_of_kin = routines.system.JDBCUtil.getString(rs_tDBInput_13, 14, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 15) {
								row4.next_of_kin_address = null;
							} else {
	                         		
        	row4.next_of_kin_address = routines.system.JDBCUtil.getString(rs_tDBInput_13, 15, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 16) {
								row4.next_of_kin_phone = null;
							} else {
	                         		
        	row4.next_of_kin_phone = routines.system.JDBCUtil.getString(rs_tDBInput_13, 16, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 17) {
								row4.next_of_kin_relationship = null;
							} else {
	                         		
        	row4.next_of_kin_relationship = routines.system.JDBCUtil.getString(rs_tDBInput_13, 17, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 18) {
								row4.date_registration = null;
							} else {
										
			row4.date_registration = routines.system.JDBCUtil.getDate(rs_tDBInput_13, 18);
		                    }
							if(colQtyInRs_tDBInput_13 < 19) {
								row4.archived = 0;
							} else {
		                          
            row4.archived = rs_tDBInput_13.getInt(19);
            if(rs_tDBInput_13.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
			...
			row4.last_modified = routines.system.JDBCUtil.getDate(rs_tDBInput_13, 20);
		                    }
							if(colQtyInRs_tDBInput_13 < 21) {
								row4.facility_id = 0;
							} else {
		                          
            row4.facility_id = rs_tDBInput_13.getLong(21);
            if(rs_tDBInput_13.wasNull()){
                    throw new RuntimeException("Null value in non-Nullable column");
            }
		                    }
        	...
        	row4.facility_name = routines.system.JDBCUtil.getString(rs_tDBInput_13, 22, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 23) {
								row4.lga_name = null;
							} else {
	                         		
        	row4.lga_name = routines.system.JDBCUtil.getString(rs_tDBInput_13, 23, false);
		                    }
							if(colQtyInRs_tDBInput_13 < 24) {
								row4.datim_id = null;
							} else {
	                         		
        	row4.datim_id = routines.system.JDBCUtil.getString(rs_tDBInput_13, 24, false);
		                    }
...
pstmt_tDBOutput_4.setNull(1, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_4.setString(1, row4.uuid);
}
            ...
            if(checkCount_tDBOutput_4 > 0) {
                        pstmtUpdate_tDBOutput_4.setLong(1, row4.id);

                        if(row4.hospital_num == null) {
pstmtUpdate_tDBOutput_4.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(2, row4.hospital_num);
}

                        if(row4.surname == null) {
pstmtUpdate_tDBOutput_4.setNull(3, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(3, row4.surname);
}

                        if(row4.other_names == null) {
pstmtUpdate_tDBOutput_4.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(4, row4.other_names);
}

                        if(row4.gender == null) {
pstmtUpdate_tDBOutput_4.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(5, row4.gender);
}

                        if(row4.date_birth != null) {
pstmtUpdate_tDBOutput_4.setTimestamp(6, new java.sql.Timestamp(row4.date_birth.getTime()));
} else {
pstmtUpdate_tDBOutput_4.setNull(6, java.sql.Types.TIMESTAMP);
}

                        if(row4.dob_estimated == null) {
pstmtUpdate_tDBOutput_4.setNull(7, java.sql.Types.BOOLEAN);
} else {pstmtUpdate_tDBOutput_4.setBoolean(7, row4.dob_estimated);
}

                        if(row4.education == null) {
pstmtUpdate_tDBOutput_4.setNull(8, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(8, row4.education);
}

                        if(row4.marital_status == null) {
pstmtUpdate_tDBOutput_4.setNull(9, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(9, row4.marital_status);
}

                        if(row4.occupation == null) {
pstmtUpdate_tDBOutput_4.setNull(10, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(10, row4.occupation);
}

                        if(row4.address == null) {
pstmtUpdate_tDBOutput_4.setNull(11, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(11, row4.address);
}

                        if(row4.phone == null) {
pstmtUpdate_tDBOutput_4.setNull(12, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(12, row4.phone);
}

                        if(row4.next_of_kin == null) {
pstmtUpdate_tDBOutput_4.setNull(13, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(13, row4.next_of_kin);
}

                        if(row4.next_of_kin_address == null) {
pstmtUpdate_tDBOutput_4.setNull(14, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(14, row4.next_of_kin_address);
}

                        if(row4.next_of_kin_phone == null) {
pstmtUpdate_tDBOutput_4.setNull(15, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(15, row4.next_of_kin_phone);
}

                        if(row4.next_of_kin_relationship == null) {
pstmtUpdate_tDBOutput_4.setNull(16, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(16, row4.next_of_kin_relationship);
}

                        if(row4.date_registration != null) {
pstmtUpdate_tDBOutput_4.setTimestamp(17, new java.sql.Timestamp(row4.date_registration.getTime()));
} else {
pstmtUpdate_tDBOutput_4.setNull(17, java.sql.Types.TIMESTAMP);
}

                        pstmtUpdate_tDBOutput_4.setInt(18, row4.archived);

                        if(row4.last_modified != null) {
pstmtUpdate_tDBOutput_4.setTimestamp(19, new java.sql.Timestamp(row4.last_modified.getTime()));
} else {
pstmtUpdate_tDBOutput_4.setNull(19, java.sql.Types.TIMESTAMP);
}

                        pstmtUpdate_tDBOutput_4.setLong(20, row4.facility_id);

                        if(row4.facility_name == null) {
pstmtUpdate_tDBOutput_4.setNull(21, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(21, row4.facility_name);
}

                        if(row4.lga_name == null) {
pstmtUpdate_tDBOutput_4.setNull(22, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(22, row4.lga_name);
}

                        if(row4.datim_id == null) {
pstmtUpdate_tDBOutput_4.setNull(23, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(23, row4.datim_id);
}

                        if(row4.uuid == null) {
pstmtUpdate_tDBOutput_4.setNull(24 + count_tDBOutput_4, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(24 + count_tDBOutput_4, row4.uuid);
}
                ...
                try {
                    ...
                    whetherReject_tDBOutput_4 = true;
                        ...
                        pstmtInsert_tDBOutput_4.setLong(1, row4.id);

                        if(row4.hospital_num == null) {
pstmtInsert_tDBOutput_4.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(2, row4.hospital_num);
}

                        if(row4.uuid == null) {
pstmtInsert_tDBOutput_4.setNull(3, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(3, row4.uuid);
}

                        if(row4.surname == null) {
pstmtInsert_tDBOutput_4.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(4, row4.surname);
}

                        if(row4.other_names == null) {
pstmtInsert_tDBOutput_4.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(5, row4.other_names);
}

                        if(row4.gender == null) {
pstmtInsert_tDBOutput_4.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(6, row4.gender);
}

                        if(row4.date_birth != null) {
pstmtInsert_tDBOutput_4.setTimestamp(7, new java.sql.Timestamp(row4.date_birth.getTime()));
} else {
pstmtInsert_tDBOutput_4.setNull(7, java.sql.Types.TIMESTAMP);
}

                        if(row4.dob_estimated == null) {
pstmtInsert_tDBOutput_4.setNull(8, java.sql.Types.BOOLEAN);
} else {pstmtInsert_tDBOutput_4.setBoolean(8, row4.dob_estimated);
}

                        if(row4.education == null) {
pstmtInsert_tDBOutput_4.setNull(9, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(9, row4.education);
}

                        if(row4.marital_status == null) {
pstmtInsert_tDBOutput_4.setNull(10, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(10, row4.marital_status);
}

                        if(row4.occupation == null) {
pstmtInsert_tDBOutput_4.setNull(11, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(11, row4.occupation);
}

                        if(row4.address == null) {
pstmtInsert_tDBOutput_4.setNull(12, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(12, row4.address);
}

                        if(row4.phone == null) {
pstmtInsert_tDBOutput_4.setNull(13, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(13, row4.phone);
}

                        if(row4.next_of_kin == null) {
pstmtInsert_tDBOutput_4.setNull(14, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(14, row4.next_of_kin);
}

                        if(row4.next_of_kin_address == null) {
pstmtInsert_tDBOutput_4.setNull(15, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(15, row4.next_of_kin_address);
}

                        if(row4.next_of_kin_phone == null) {
pstmtInsert_tDBOutput_4.setNull(16, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(16, row4.next_of_kin_phone);
}

                        if(row4.next_of_kin_relationship == null) {
pstmtInsert_tDBOutput_4.setNull(17, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(17, row4.next_of_kin_relationship);
}

                        if(row4.date_registration != null) {
pstmtInsert_tDBOutput_4.setTimestamp(18, new java.sql.Timestamp(row4.date_registration.getTime()));
} else {
pstmtInsert_tDBOutput_4.setNull(18, java.sql.Types.TIMESTAMP);
}

                        pstmtInsert_tDBOutput_4.setInt(19, row4.archived);

                        if(row4.last_modified != null) {
pstmtInsert_tDBOutput_4.setTimestamp(20, new java.sql.Timestamp(row4.last_modified.getTime()));
} else {
pstmtInsert_tDBOutput_4.setNull(20, java.sql.Types.TIMESTAMP);
}

                        pstmtInsert_tDBOutput_4.setLong(21, row4.facility_id);

                        if(row4.facility_name == null) {
pstmtInsert_tDBOutput_4.setNull(22, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(22, row4.facility_name);
}

                        if(row4.lga_name == null) {
pstmtInsert_tDBOutput_4.setNull(23, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(23, row4.lga_name);
}

                        if(row4.datim_id == null) {
pstmtInsert_tDBOutput_4.setNull(24, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_4.setString(24, row4.datim_id);
}
...
System.out.println("*****************************************************************");
System.out.println("***********PrEP PERSON MIGRATION REPORT START*****************");
java.time.LocalDateTime startTime = java.time.LocalDateTime.now();
globalMap.put("startTime", startTime);
System.out.println("Start time - "+ startTime);
...
int count_tDBOutput_1=0;
                                ...
                                if(!whetherExist_tDBOutput_1) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                                        stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1 + "\"(\"id\" SERIAL  not null ,\"hospital_number\" VARCHAR(25)   not null ,\"uuid\" VARCHAR(36)  ,\"first_name\" VARCHAR(230)   not null ,\"surname\" VARCHAR(45)  ,\"other_name\" VARCHAR(75)  ,\"" + "gender" + "\" VARCHAR(50),\"date_of_birth\" DATE ,\"is_date_of_birth_estimated\" BOOL ,\"" + "education" + "\" VARCHAR(50),\"" + "marital_status" + "\" VARCHAR(50),\"" + "employment_status" + "\" VARCHAR(50),\"" + "address" + "\" VARCHAR(50),\"" + "contact_point" + "\" VARCHAR(50),\"" + "contact" + "\" VARCHAR(50),\"date_of_registration\" DATE  not null ,\"archived\" INT4 ,\"active\" BOOL default true  not null ,\"deceased\" BOOL default false ,\"deceased_date_time\" TIMESTAMP ,\"created_date\" TIMESTAMP ,\"last_modified_date\" TIMESTAMP(29)  ,\"created_by\" VARCHAR ,\"last_modified_by\" VARCHAR ,\"" + "identifier" + "\" VARCHAR(50),\"facility_id\" INT8  not null ,\"" + "organization" + "\" VARCHAR(50),\"sex\" TEXT ,\"full_name\" VARCHAR(100)   not null ,primary key(\"uuid\",\"sex\"))");
                                    }
                                }
 					    ...
 					    boolean forceLooprow6 = false;
 							...
 							row6Struct row6ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_9 = false;
                        		    		    ...
                        		    		    row6HashKey.uuid = Unique_Patients.uuid ;
...
final_mapper_tmp.id = row6.id != 0 ? row6.id : Numeric.sequence("s1",row5.max + Unique_Patients.id,1) ;
...
final_mapper_tmp.deceased = false;
...
final_mapper_tmp.created_by = "Lamis data migration system";
final_mapper_tmp.last_modified_by = "Lamis data migration system";
 ...
 */
    ...
    public static void main(String[] args){
        final PrEP_Patient_H2 PrEP_Patient_H2Class = new PrEP_Patient_H2();

        int exitCode = PrEP_Patient_H2Class.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = PrEP_Patient_H2.class.getClassLoader().getResourceAsStream("lamisplus_etl/prep_patient_h2_6_0/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = PrEP_Patient_H2.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : PrEP_Patient_H2");
        }
...
/************************************************************************************************
 *     868472 characters generated by Talend Open Studio for Big Data 
 *     on the May 4, 2023 2:02:51 PM WAT
 ************************************************************************************************/