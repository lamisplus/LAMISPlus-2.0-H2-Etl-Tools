...
package lamisplus_etl.patient_person_h2_6_0;
...
/**
 * Job: Patient_Person_H2 Purpose: Patient Workflow with validated FHIR R4 and JSON Fields and Codeset transfomations<br>
 * Description: Validate JSON Fields and Codeset transfomation <br>
 * @author ilozuechukwuemeka@yahoo.com
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class Patient_Person_H2 implements TalendJob {
...
public java.lang.String getLamish2_Password(){
	...
	private final String jobName = "Patient_Person_H2";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Patient_Person_H2.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Patient_Person_H2.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class CreatJSON_FieldStruct implements routines.system.IPersistableRow<CreatJSON_FieldStruct> {
    final static byte[] commonByteArrayLock_LAMISPLUS_ETL_Patient_Person_H2 = new byte[0];
    static byte[] commonByteArray_LAMISPLUS_ETL_Patient_Person_H2 = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_LAMISPLUS_ETL_Patient_Person_H2.length) {
				if(length < 1024 && commonByteArray_LAMISPLUS_ETL_Patient_Person_H2.length == 0) {
   					commonByteArray_LAMISPLUS_ETL_Patient_Person_H2 = new byte[1024];
				} else {
   					commonByteArray_LAMISPLUS_ETL_Patient_Person_H2 = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_LAMISPLUS_ETL_Patient_Person_H2, 0, length);
			strReturn = new String(commonByteArray_LAMISPLUS_ETL_Patient_Person_H2, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_LAMISPLUS_ETL_Patient_Person_H2) {
 ...
 */
	...
	final String decryptedPassword_tDBInput_11 = context.LAMIS3_Password; 
		    ...
		    String dbquery_tDBInput_11 = "SELECT p.patient_id as patient_id, id_uuid as person_uuid,\np.gender,p.marital_status,\n    p.education,p.occupation,\n"
+"	p.relation_kin as next_of_kin_relationship\nFROM patient p\nJOIN facility ndrf ON ndrf.facility_id = p.facility_id\nJOI"
+"N\npublic.dblink\n('lamis','SELECT base_org.name, base_org.id, org.code\nFROM base_organisation_unit base_org\nINNER JOI"
+"N base_organisation_unit_identifier org\nON org.organisation_unit_id=base_org.id WHERE organisation_unit_level_id=4'\n)"
...
if(dbschema_tDBOutput_4 == null || dbschema_tDBOutput_4.trim().length() == 0) {
	tableName_tDBOutput_4 = ("etl_patient_stage_2");
} else {
	tableName_tDBOutput_4 = dbschema_tDBOutput_4 + "\".\"" + ("etl_patient_stage_2");
}
...
int count_tDBOutput_4=0;
                                ...
                                try (java.sql.ResultSet rsTable_tDBOutput_4 = dbMetaData_tDBOutput_4.getTables(null, null, null, new String[]{"TABLE"})) {
                                    ...
                                    while(rsTable_tDBOutput_4.next()) {
                                        ...
                                        if(table_tDBOutput_4.equals(("etl_patient_stage_2"))
                                ...
                                if(!whetherExist_tDBOutput_4) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_4 = conn_tDBOutput_4.createStatement()) {
                                        stmtCreate_tDBOutput_4.execute("CREATE TABLE \"" + tableName_tDBOutput_4 + "\"(\"id\" INT8  not null ,\"hospital_num\" VARCHAR(25)   not null ,\"uuid\" VARCHAR(100)   not null ,\"surname\" TEXT ,\"other_names\" TEXT ,\"gender\" VARCHAR(7)  ,\"date_birth\" DATE ,\"dob_estimated\" BOOL ,\"education\" VARCHAR(25)  ,\"marital_status\" VARCHAR(15)  ,\"occupation\" VARCHAR(25)  ,\"address\" TEXT ,\"phone\" TEXT ,\"next_of_kin\" TEXT ,\"next_of_kin_address\" TEXT ,\"next_of_kin_phone\" TEXT ,\"next_of_kin_relationship\" VARCHAR(25)  ,\"date_registration\" DATE  not null ,\"archived\" INT4  not null ,\"last_modified\" TIMESTAMP(29)  ,\"facility_id\" INT8  not null ,\"facility_name\" VARCHAR(100)   not null ,\"lga_name\" VARCHAR(45)   not null ,\"datim_id\" VARCHAR(45)   not null ,primary key(\"id\"))");
                                    }
                                }
	    ...
	    String update_tDBOutput_4 = "UPDATE \"" + tableName_tDBOutput_4 + "\" SET \"hospital_num\" = ?,\"uuid\" = ?,\"surname\" = ?,\"other_names\" = ?,\"gender\" = ?,\"date_birth\" = ?,\"dob_estimated\" = ?,\"education\" = ?,\"marital_status\" = ?,\"occupation\" = ?,\"address\" = ?,\"phone\" = ?,\"next_of_kin\" = ?,\"next_of_kin_address\" = ?,\"next_of_kin_phone\" = ?,\"next_of_kin_relationship\" = ?,\"date_registration\" = ?,\"archived\" = ?,\"last_modified\" = ?,\"facility_id\" = ?,\"facility_name\" = ?,\"lga_name\" = ?,\"datim_id\" = ? WHERE \"id\" = ?";
 ...
 */
	...
	final String decryptedPassword_tDBInput_13 = context.LAMIS3_Password; 
		    ...
		    String dbquery_tDBInput_13 = "SELECT DISTINCT ps.patient_id as id,ps.hospital_num, ps.id_uuid as uuid,\ninitcap(replace(replace(replace(replace(repla"
+"ce(replace(replace(replace(replace(replace(replace(replace(ps.surname, '*', 'e'), '`', 'c'), '~', 'b'), '^', 'a'), '{', "
+"'o'), '}', 'n'), '|', 'k'), '%', 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) as surname,\ninitcap(replace(replace("
+"replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(ps.other_names, '*', 'e'), '`', 'c'), '~"
+"', 'b'), '^', 'a'), '{', 'o'), '}', 'n'), '|', 'k'), '%', 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) as other_nam"
+"es,\nps.gender, ps.date_birth,\n'0'::BOOLEAN as dob_estimated,\nps.education,ps.marital_status,ps.occupation,\ninitcap(r"
+"eplace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(ps.address, '*', 'e'), '`"
+"', 'c'), '~', 'b'), '^', 'a'), '{', 'o'), '}', 'n'), '|', 'k'), '%', 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) a"
+"s address,\nreplace(replace(replace(replace(replace(replace(replace(replace(ps.phone, '!', '9'), '@', '8'), '#', '7'), '"
+"$', '6'), '*', '5'), '`', '3'),'~', '2'),'^', '1') as phone,\ninitcap(replace(replace(replace(replace(replace(replace(re"
+"place(replace(replace(replace(replace(replace(ps.next_kin, '*', 'e'), '`', 'c'), '~', 'b'), '^', 'a'), '{', 'o'), '}', '"
+"n'), '|', 'k'), '%', 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) as next_of_kin,\ninitcap(replace(replace(replace("
+"replace(replace(replace(replace(replace(replace(replace(replace(replace(ps.address_kin, '*', 'e'), '`', 'c'), '~', 'b'),"
+" '^', 'a'), '{', 'o'), '}', 'n'), '|', 'k'), '%', 'j'), '!', 'i'), '@', 'h'), '#', 'g'), '$', 'f')) as next_of_kin_addre"
+"ss,\nreplace(replace(replace(replace(replace(replace(replace(replace(ps.phone_kin, '!', '9'), '@', '8'), '#', '7'), '$',"
+" '6'), '*', '5'), '`', '3'),'~', '2'),'^', '1') as next_of_kin_phone,\n    ps.relation_kin as next_of_kin_relationship,p"
+"s.date_registration,\n	'0'::INTEGER archived,\n	ps.time_stamp as last_modified,\n    ps.facility_id,fc.name AS facility_"
+"name,lg.name AS lga_name, fc.datim_id AS datim_id\n  FROM public.patient ps\n  INNER JOIN facility fc\n  ON ps.facility_"
+"id=fc.facility_id\n  INNER JOIN lga lg\n  ON fc.lga_id=lg.lga_id";
 ...
 */
	...
	currentComponent="tDBOutput_4";
        ...
        whetherReject_tDBOutput_4 = false;
                    pstmt_tDBOutput_4.setLong(1, row4.id);
...
} else {pstmtUpdate_tDBOutput_4.setString(1, row4.hospital_num);
}

                        if(row4.uuid == null) {
pstmtUpdate_tDBOutput_4.setNull(2, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_4.setString(2, row4.uuid);
}
...
pstmtUpdate_tDBOutput_4.setNull(23, java.sql.Types.VARCHAR);
                        ...
                        pstmtUpdate_tDBOutput_4.setLong(24 + count_tDBOutput_4, row4.id);
...
int count_tDBOutput_1=0;
                                ...
                                if(!whetherExist_tDBOutput_1) {
                                    try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                                        stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1 + "\"(\"id\" SERIAL  not null ,\"hospital_number\" VARCHAR(25)   not null ,\"uuid\" VARCHAR(36)  ,\"first_name\" VARCHAR(230)  ,\"surname\" VARCHAR(45)  ,\"other_name\" VARCHAR(75)  ,\"" + "gender" + "\" VARCHAR(50),\"date_of_birth\" DATE ,\"is_date_of_birth_estimated\" BOOL ,\"" + "education" + "\" VARCHAR(50),\"" + "marital_status" + "\" VARCHAR(50),\"" + "employment_status" + "\" VARCHAR(50),\"" + "address" + "\" VARCHAR(50),\"" + "contact_point" + "\" VARCHAR(50),\"" + "contact" + "\" VARCHAR(50),\"date_of_registration\" DATE  not null ,\"archived\" INT4 ,\"active\" BOOL default true  not null ,\"deceased\" BOOL default false ,\"deceased_date_time\" TIMESTAMP ,\"created_date\" TIMESTAMP ,\"last_modified_date\" TIMESTAMP(29)  ,\"created_by\" VARCHAR ,\"last_modified_by\" VARCHAR ,\"" + "identifier" + "\" VARCHAR(50),\"facility_id\" INT8  not null ,\"" + "organization" + "\" VARCHAR(50),\"sex\" TEXT ,\"full_name\" VARCHAR(100)  ,primary key(\"id\",\"sex\"))");
                                    }
                                }
...
final_mapper_tmp.created_by = "ETL";
final_mapper_tmp.last_modified_by = "ETL";
 ...
 */
    ...
    public static void main(String[] args){
        final Patient_Person_H2 Patient_Person_H2Class = new Patient_Person_H2();

        int exitCode = Patient_Person_H2Class.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Patient_Person_H2.class.getClassLoader().getResourceAsStream("lamisplus_etl/patient_person_h2_6_0/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Patient_Person_H2.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Patient_Person_H2");
        }
...
/************************************************************************************************
 *     839509 characters generated by Talend Open Studio for Big Data 
 *     on the March 22, 2023 12:15:17 PM WAT
 ************************************************************************************************/